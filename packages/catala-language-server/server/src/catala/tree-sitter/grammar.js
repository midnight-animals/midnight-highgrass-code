...
module.exports = grammar({
  ...
  rules: {
    ...
    typ_list: ($) => seq(repeat(seq($.typ, $.COMMA)), $.typ),
    typ: ($) =>
      choice(
        $.primitive_typ,
        seq($.LIST, $.OF, $.typ),
    ...
    _path: ($) =>
      choice(seq($.module_name, $.DOT), seq($._path, $.module_name, $.DOT)),
    ...
    qfield: ($) => seq(optional(seq($.qenum_struct, $.DOT)), $.field_name),

    var_list: ($) => seq(repeat(seq($.variable, $.COMMA)), $.variable),

    binder: ($) => choice($.variable, seq($.LPAREN, $.var_list, $.RPAREN)),
    ...
    e_variable: ($) => prec.right(seq(optional($._path), $.variable)),

    tuple_contents: ($) => seq(repeat(seq($._expr, $.COMMA)), $._expr),

    e_tuple: ($) => seq($.LPAREN, $.tuple_contents, $.RPAREN),

    collection_elements: ($) => seq(repeat(seq($._expr, $.SEMICOLON)), $._expr),
    ...
    fun_arguments: ($) =>
      prec.right(seq(repeat(seq($.fun_argument, $.COMMA)), $.fun_argument)),
    e_apply: ($) =>
      prec.right(
        'apply',
        seq(field('fun', $._expr), $.OF, field('args', $.fun_arguments))
      ),
    e_scope_apply: ($) =>
      prec.right(
        'apply',
        seq(
          ...
          $.qscope,
          optional(
            seq($.WITH, $.LBRACE, optional($.struct_content_fields), $.RBRACE)
          )
        )
      ),
    e_test_match: ($) =>
      prec.right(
        ...
        seq(
          ...
          $.WITH_PATT,
          $.qconstructor,
          optional(seq($.OF, $.variable))
        )
      ),
    e_coll_contains: ($) =>
      prec.right(
        'apply',
        seq(field('coll', $._expr), $.CONTAINS, field('elt', $._expr))
      ),
    e_coll_sum: ($) =>
      prec.right(
        'apply',
        seq($.SUM, $.primitive_typ, $.OF, field('coll', $._expr))
      ),
    e_coll_map: ($) =>
      prec.right(
        'apply',
    ...
    e_unop: ($) =>
      prec.right(
        'unop_expr',
        choice(seq($.NOT, $._expr), seq($.MINUS, $._expr))
      ),
    ...
    e_binop: ($) =>
      choice(
        prec.left(
          'MULT',
          seq(field('lhs', $._expr), field('op', $.MULT), field('rhs', $._expr))
        ),
        prec.left(
          'MULT',
          seq(field('lhs', $._expr), field('op', $.DIV), field('rhs', $._expr))
        ),
        prec.left(
          'PLUS',
          seq(field('lhs', $._expr), field('op', $.PLUS), field('rhs', $._expr))
        ),
        prec.left(
          'PLUS',
          seq(
        ...
        prec.right(
          'AND',
          seq(field('lhs', $._expr), field('op', $.AND), field('rhs', $._expr))
        ),
        prec.right(
          'AND',
          seq(field('lhs', $._expr), field('op', $.OR), field('rhs', $._expr))
        ),
        prec.right(
          'AND',
          seq(field('lhs', $._expr), field('op', $.XOR), field('rhs', $._expr))
        )
      ),
    ...
    match_case: ($) =>
      prec.right(
        seq(
          choice(
            $.WILDCARD,
            seq($.qconstructor, optional(seq($.OF, $.binder)))
          ),
          $.COLON,
          $._expr
        )
    ...
    e_fieldaccess: ($) => prec.left('DOT', seq($._expr, $.DOT, $.qfield)),
    ...
    struct_content_field: ($) => seq($.qfield, $.COLON, $._expr),
    struct_content_fields: ($) => repeat1(seq($.ALT, $.struct_content_field)),
    ...
    params_decl: ($) => seq(repeat(seq($.param_decl, $.COMMA)), $.param_decl),
    _depends_stance: ($) =>
      seq(
        $.DEPENDS,
        choice(seq($.LPAREN, $.params_decl, $.RPAREN), $.params_decl)
      ),
    scope_decl_item: ($) =>
      seq(
        optional($.scope_decl_item_attribute),
        choice(
          seq(
            $.variable,
    ...
    struct_decl_item: ($) =>
      seq(
        choice(
          seq($.DATA, $.field_name, $.CONTENT, $.typ),
    ...
    code_block: ($) =>
      seq(
        choice($.BEGIN_CODE, $.BEGIN_METADATA),
        optional($._code),
        $.END_CODE
      ),
      ...
      seq(
        ...
        choice(
          ...
          seq(
            $.MODULE_USE,
            $.module_name,
            optional(seq($.MODULE_ALIAS, $.module_name))
          )
        ),
        $.END_DIRECTIVE
      ),
    ...
    law_heading: ($) =>
      prec(1, seq($.LAW_HEADING, prec(1, optional(seq($.BAR, $.LAW_LABEL))))),
    ...
    PLUS: ($) =>
      token(seq(tokens.PLUS, token.immediate(tokens.OP_KIND_SUFFIX))),
    MINUS: ($) =>
      token(seq(tokens.MINUS, token.immediate(tokens.OP_KIND_SUFFIX))),
    MULT: ($) =>
      token(seq(tokens.MULT, token.immediate(tokens.OP_KIND_SUFFIX))),
    DIV: ($) => token(seq(tokens.DIV, token.immediate(tokens.OP_KIND_SUFFIX))),

    EQUAL: ($) => token(tokens.EQUAL),
    NOT_EQUAL: ($) => token(tokens.NOT_EQUAL),
    GREATER: ($) =>
      token(seq(tokens.GREATER, token.immediate(tokens.OP_KIND_SUFFIX))),
    GREATER_EQUAL: ($) =>
      token(seq(tokens.GREATER_EQUAL, token.immediate(tokens.OP_KIND_SUFFIX))),
    LESSER: ($) =>
      token(seq(tokens.LESSER, token.immediate(tokens.OP_KIND_SUFFIX))),
    LESSER_EQUAL: ($) =>
      token(seq(tokens.LESSER_EQUAL, token.immediate(tokens.OP_KIND_SUFFIX))),
